...
async function randomString(len) {
  len = len || 6;
  let $chars = 'ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678'; // Default removed easily confused characters oOLl,9gq,Vv,Uu,I1
...
async function handleRequest(request) {
  ...
  if (request.method === "POST") {
    ...
    console.log(stat)
    if (typeof (stat) == "undefined") {
      return new Response(`{"status":200,"key":"https://cnnb.us.kg/` + random_key + `"}`, {
        headers: response_header,
      })
    } else {
  ...
  if (!path) {
    ...
    const html = await fetch("https://short-links-web.pages.dev/")
